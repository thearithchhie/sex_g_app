import 'package:sex_g_app/export.dart';
import 'package:easy_debounce/easy_debounce.dart';
import 'package:easy_localization/easy_localization.dart';
import 'package:modal_bottom_sheet/modal_bottom_sheet.dart';

class CustomPhoneField extends StatelessWidget {
  final String hintText;
  final TextEditingController? controller;
  final bool isOutLineBorder;
  final String? label;
  final bool isVerify;

  const CustomPhoneField({
    super.key,
    this.hintText = "Enter your text",
    this.controller,
    this.isOutLineBorder = false,
    this.label,
    this.isVerify = false,
  });

  static const String route = "/CustomTextField";

  @override
  Widget build(BuildContext context) {
    final InputBorder bordered = OutlineInputBorder(
      borderSide: isOutLineBorder
          ? BorderSide(
              color: app.isDark ? AppColor.pr900 : const Color(0xffA4A4A4),
              width: 1)
          : BorderSide.none,
      borderRadius: BorderRadius.circular(12),
    );
    final GetCountryCubit _getCountryCubit = context.read<GetCountryCubit>();
    if (_getCountryCubit.state.response.result == null) {
      _getCountryCubit.getCountry();
    }
    return Column(
      mainAxisSize: MainAxisSize.min,
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (label != null)
          Container(
            margin: 5.pb(),
            child: Text(
              label!,
              style: context.typography.body14B
                  .copyWith(color: app.isDark ? AppColor.primary : null),
            ),
          ),
        TextFormField(
          controller: controller,
          style: context.typography.body12R,
          validator: (value) {
            if (value == null || value.isEmpty) {
              return hintText.localize();
            }
            return null;
          },
          decoration: InputDecoration(
            hintText: hintText.tr(),
            prefixIcon: InkWell(
              highlightColor: Colors.transparent,
              splashColor: Colors.transparent,
              onTap: () {
                showCountryPicker(context);
              },
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  12.widthBox,
                  BlocBuilder<GetCountryCubit, GetCountryState>(
                    builder: (context, state) {
                      return Text(
                        "+${state.countryCodeSelected?.cid ?? ""}",
                        style: context.typography.body12R.copyWith(
                            color: app.isDark ? AppColor.pr400 : null),
                        // style: const TextStyle(color: AppColor.pr400, fontSize: 14, fontWeight: FontWeight.w500),
                      );
                    },
                  ),
                  Icon(Icons.keyboard_arrow_down,
                      color: app.isDark ? AppColor.primary : null, size: 20),
                ],
              ),
            ),
            errorStyle:
                context.typography.body12R.copyWith(color: AppColor.danger),
            contentPadding: const EdgeInsets.all(15),
            hintStyle: context.typography.body12R.copyWith(
                color: app.isDark ? AppColor.pr400 : AppColor.neutral300),
            filled: !isOutLineBorder,
            fillColor: const Color(0xff353979),
            errorBorder: bordered,
            focusedBorder: bordered,
            focusedErrorBorder: bordered,
            disabledBorder: bordered,
            enabledBorder: bordered,
            border: bordered,
            suffixIcon: isVerify
                ? Container(
                    padding: 10.px(),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      spacing: 3,
                      children: [
                        Text('verify'.localize(),
                            style: context.typography.body12R),
                        const Icon(Icons.arrow_forward_ios_outlined, size: 15),
                      ],
                    ),
                  )
                : null,
          ),
          keyboardType: TextInputType.number,
        ),
      ],
    );
  }

  void showCountryPicker(BuildContext context,
      {GetCountryCubit? getCountryCubit}) {
    showMaterialModalBottomSheet(
      context: context,
      builder: (_) {
        return _PhoneNumberSelection(
            getCountryCubit:
                getCountryCubit ?? context.read<GetCountryCubit>());
      },
    );
  }
}

class _PhoneNumberSelection extends StatefulWidget {
  final GetCountryCubit getCountryCubit;

  const _PhoneNumberSelection({super.key, required this.getCountryCubit});

  @override
  State<_PhoneNumberSelection> createState() => _PhoneNumberSelectionState();
}

class _PhoneNumberSelectionState extends State<_PhoneNumberSelection> {
  @override
  void initState() {
    widget.getCountryCubit.onClearSearch();
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      height: context.height * .5,
      decoration: const BoxDecoration(
          borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      child: SafeArea(
        top: false,
        child: Column(
          children: [
            16.heightBox,
            Text("select_your_country".localize(),
                style: context.typography.body16B),
            12.heightBox,
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: CustomTextField(
                hintText: "search_country_code".localize(),
                isOutLineBorder: true,
                prefixIcon: const Icon(Icons.search, size: 20),
                onChanged: (searchText) {
                  EasyDebounce.debounce(
                      'searchCountryCode', const Duration(milliseconds: 500),
                      () {
                    widget.getCountryCubit.onSearchCountryCode(searchText);
                  });
                },
              ),
            ),
            Expanded(
              child: BlocBuilder<GetCountryCubit, GetCountryState>(
                builder: (context, state) {
                  if (state.stateStatus == AppStateStatus.loading) {
                    return const Center(
                        child: CircularProgressIndicator.adaptive());
                  }
                  if (state.stateStatus == AppStateStatus.success) {
                    List<CountryCodeRes> countries =
                        state.response.result as List<CountryCodeRes>;
                    CountryCodeRes? selected = state.countryCodeSelected;
                    return ListView.builder(
                      padding: EdgeInsets.zero,
                      itemCount: countries.length,
                      itemBuilder: (context, ind) {
                        final country = countries[ind];
                        return InkWell(
                          onTap: () => {
                            widget.getCountryCubit.onSelectCountryCode(country),
                            context.pop()
                          },
                          child: Padding(
                            padding: const EdgeInsets.only(left: 16),
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                //
                                12.heightBox,
                                Container(
                                    color: AppColor.grey.withAlpha(30),
                                    height: 1),
                                12.heightBox,
                                Row(
                                  children: [
                                    Expanded(
                                        child: Text("${country.countryName}",
                                            style: context.typography.body12R)),
                                    if (selected?.cid == country.cid)
                                      Image.asset(Assets.icons.checked.path,
                                          scale: 4),
                                    16.widthBox,
                                  ],
                                ),
                              ],
                            ),
                          ),
                        );
                      },
                    );
                  }
                  return const EmptyWidget();
                },
              ),
            ),
          ],
        ),
      ),
    );
  }
}
